...
class Booking(models.Model):
    ...
    def action_view_invoice(self):
        ...
        if self.payment_status != "paid":
            raise UserError("Invoice hanya dapat dilihat jika pembayaran sudah dilakukan.")
        if not self.invoice_id:
            raise UserError("Invoice belum tersedia.")
        return {
    ...
    def create(self, vals):
        ...
        if customer_id:
            existing_unpaid = self.search(
                [("customer_id", "=", customer_id), ("payment_status", "=", "unpaid")],
                limit=1,
            )
            if existing_unpaid:
                raise UserError("Customer ini masih memiliki booking yang belum dibayar.")
    ...
    def get_booking_by_customer(self, start_date, end_date):
        ...
        for rec in bookings:
            ...
            result.append({"name": customer.name, "value": rec["customer_id_count"]})
        return result
    ...
    def get_booking_status_count(self, start_date, end_date):
        domain = [("booking_date", ">=", start_date), ("booking_date", "<=", end_date)]
        data = self.read_group(domain=domain, fields=["status"], groupby=["status"])
        status_map = dict(self._fields["status"].selection)
        result = []
        for rec in data:
            result.append(
                {"name": status_map.get(rec["status"], "Tidak diketahui"), "value": rec["status_count"]}
            )
        return result